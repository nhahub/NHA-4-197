...
import { analyzePollution, ApiError } from "@/lib/api";
...
export default function PollutionPage() {
  ...
  async function handleAnalyze() {
    ...
    setResult(null);
    try {
      const res = await analyzePollution(file);
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-16">
      <span className="font-mono text-xs uppercase tracking-widest text-fern">
        Computer vision · POST /api/analyze-pollution
      </span>
      <h1 className="mt-3 font-display text-4xl font-medium text-canopy">
        Pollution Scan
      </h1>
      <p className="mt-3 max-w-xl text-ink/70">
        Upload a photo of a scene — a river, a street, a site. GreenCycle
        estimates the pollution type, severity, and likely cause.
      </p>
